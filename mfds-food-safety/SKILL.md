...
식약처/식품안전나라 공개 표면을 **`k-skill-proxy` 경유**로 조회해 **부적합 식품 목록**과 **회수·판매중지 공개 목록**을 확인한다.
...
- `k-skill-proxy`의 `/v1/mfds/food-safety/search` route가 있는 hosted/self-host 프록시에 접근 가능할 것
...
- `FOODSAFETYKOREA_API_KEY` 는 **프록시 운영 서버** 환경에서 식품안전나라 회수 live 조회용으로만 둔다. 없으면 public sample 회수 feed로 fallback 할 수 있다.
